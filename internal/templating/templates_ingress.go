...
func GenerateIngressTemplate(
	...
	ingress.ObjectMeta.Labels = map[string]string{
		...
		"lagoon.sh/oauthProtected":     strconv.FormatBool(*route.OauthProtected),
	}
	...
	if lValues.Oauth2ProxyDomain != "" && *route.OauthProtected {
		// Protect all development and autogenerated routes with oauth2-proxy
		// see https://oauth2-proxy.github.io/oauth2-proxy/configuration/integration#configuring-for-use-with-the-nginx-auth_request-directive
		additionalAnnotations["nginx.ingress.kubernetes.io/auth-response-headers"] = "Authorization"
		additionalAnnotations["nginx.ingress.kubernetes.io/auth-signin"] = lValues.Oauth2ProxyDomain + "/oauth2/start?rd=$scheme://$host$request_uri"
		additionalAnnotations["nginx.ingress.kubernetes.io/auth-url"] = lValues.Oauth2ProxyDomain + "/oauth2/auth"
		additionalAnnotations["nginx.ingress.kubernetes.io/configuration-snippet"] = `auth_request_set $name_upstream_1 $upstream_cookie__oauth2_proxy_1;
		access_by_lua_block {
